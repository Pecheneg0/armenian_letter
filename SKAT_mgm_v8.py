...
class DroneController:
        ...
        try:
            if self.USE_PICAM:
                ...
                try:
                    ...
                    logging.info("PiCamera запущена с настройками резкости")
                    self.send_ssh_message("PiCamera запущена с настройками резкости") 
                except:
                    logging.info("PiCamera запущена без настройек резкости")
                    self.send_ssh_message("PiCamera запущена без настройек резкости")
    ...
    def execute_landing(self, offset_north, offset_east, altitude): 
       #print("\n=== Начало процедуры посадки ===")
        ...
        start_time = time.time()
        last_command_time = 0
        command_interval = 2  # Интервал между командами (сек)
    ...
    def process_aruco(self):
        ...
        logging.info(f"[{datetime.now().strftime('%H:%M:%S')}] Начальный целевой маркер: {current_target_id}")
        logging.info(f"[{datetime.now().strftime('%H:%M:%S')}] Разрешение камеры: {CAMERA_RESOLUTION_USBCAM}")
        logging.info(f"[{datetime.now().strftime('%H:%M:%S')}] Частота обработки: {PROCESSING_FPS} FPS")
