...
MIN_LOOP_DELAY = 0.02
MAX_LOOP_DELAY = 0.1
COMMAND_INTERVAL = 0.1 # интервал отправки команд (10 герц)
POSITION_UPDATE_RATE = 20 # частота обновления позиции
...
class DroneController:
            ...
            while not self.land_command_sent:
                ...
                if altitude > self.min_altitude:
                    if abs(offset_north) < self.pos_tolerance and abs(offset_east) < self.pos_tolerance:
                        ...
                        smooth_factor = 0.7
                        self.move_to_offset(
                            offset_north * smooth_factor,
                            offset_east * smooth_factor, 
                            altitude)
                    time.sleep(2)
                else:
                    if abs(offset_north) < self.pos_tolerance and abs(offset_east) < self.pos_tolerance:
                        ...
                        self.move_to_offset(offset_north * smooth_factor, 
                                            offset_east * smooth_factor,
                                            0)
        except Exception as e:
    ...
    def process_aruco(self):
        ...
        self.send_ssh_message(f"Старт поиска маркера {current_target_id} (высота > {TARGET_ALTITUDE}m)")
        ...
        while self.current_mode == MODE_ARUCO:
            ...
            if current_time - last_position_update >= 1/POSITION_UPDATE_RATE:
                gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                ...
                if ids is not None and current_target_id in ids.flatten():
                    ...
                    if current_target_id == MARKER_42 and altitude <= TARGET_ALTITUDE:
                        ...
                        self.send_ssh_message(f"Переключение на маркер {current_target_id} (высота ≤ {TARGET_ALTITUDE}m)")
                        continue
            ...
            remaining_delay = max(0, MIN_LOOP_DELAY - loop_time)
            time.sleep(min(remaining_delay, MAX_LOOP_DELAY))
        ...
        try:
            while True:
                ...
                try:  # Внутренний обработчик ошибок
                    ...
                    sleep_time = max(0, MIN_LOOP_DELAY - loop_time)
                    time.sleep(min(sleep_time, MAX_LOOP_DELAY))
