...
VIDEO_FPS = 60  # Частота кадров для видео
...
class VideoRecorder:
    ...
    def start(self, filename, resolution, fps):
        if self.recording:
            self.stop()
            
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        self.filename = os.path.join(VIDEO_DIR, filename)
        self.writer = cv2.VideoWriter(self.filename, fourcc, fps, resolution)
        self.recording = True
...
class DroneController:
            ...
            if self.USE_PICAM:
                ...
                config = self.camera.create_preview_configuration(
                    ...
                    controls={
                        ...
                        "AwbEnable": False,  # Отключаем авто баланс белого
                        "AwbMode": 0,       # Режим "Auto" (0) или конкретный режим освещения
                        "AeEnable": False,   # Отключаем авто экспозицию
                        "ExposureTime": 10000,  # В микросекундах (начните с 10000 и регулируйте)
                        "AnalogueGain": 1.0,    # Минимальное усиление
                        "Brightness": 0.1,      # 0-1 (рекомендуется 0.1-0.3)
                        "Contrast": 1.2,        # 1.0 = нормальный, >1.0 увеличивает контраст
                        "Saturation": 0.8,      # 0.0-2.0 (меньше = менее насыщенные цвета)
                        "Sharpness": 1.5,       # 0.0-16.0 (умеренная резкость)
                    }
                )
                ...
                self.camera.set(cv2.CAP_PROP_AUTO_EXPOSURE, 1)      # 1 = manual exposure
                self.camera.set(cv2.CAP_PROP_EXPOSURE, 50)          # Чем меньше, тем темнее (10-100)
                self.camera.set(cv2.CAP_PROP_AUTO_WB, 0)            # Отключаем авто баланс белого
                self.camera.set(cv2.CAP_PROP_WB_TEMPERATURE, 4500)  # Ручная установка баланса белого
                self.camera.set(cv2.CAP_PROP_GAIN, 10)              # Минимальное усиление (0-100)
                self.camera.set(cv2.CAP_PROP_BRIGHTNESS, 10)        # 0-100 (рекомендуется 10-30)
                self.camera.set(cv2.CAP_PROP_CONTRAST, 50)          # 0-100 (рекомендуется 40-60)
                self.camera.set(cv2.CAP_PROP_SATURATION, 40)         # 0-100 (меньше = меньше засветки)
                self.camera.set(cv2.CAP_PROP_SHARPNESS, 30)          # 0-100 (умеренная резкость)
    ...
    def start_video_recording(self, mode_name):
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"{mode_name}_{timestamp}.mp4"
        self.current_video_file = self.video_recorder.start(
            filename, 
            VIDEO_RESOLUTION, 
            VIDEO_FPS
        )
        self.send_ssh_message(f"Начата видеозапись: {self.current_video_file}")
        
    def stop_video_recording(self):
        if self.video_recorder.recording:
            filename = self.video_recorder.stop()
            self.send_ssh_message(f"Видеозапись сохранена: {filename}")
            return filename
        return ""
    ...
    def process_aruco(self):
        ...
        try:
            self.start_video_recording("ARUCO")
    ...
    def process_letters(self):
        ...
        try:
            self.start_video_recording("LETTERS")
