...
import time
import torch
# Импортируем mavutil, но будем подменять его соединение
...
# from libcamera import controls # Раскомментируйте, если есть проблемы с импортом на PC, но на Pi нужно
...
# ===== МОДУЛЬ ЭМУЛЯЦИИ MAVLINK (ДЛЯ ТЕСТА БЕЗ ДРОНА) =====
class MockMavlink:
    def __init__(self):
        self.target_system = 1
        self.mav = self 
        
        # Начальные координаты (Москва, полигон)
        self.sim_lat = 55.754066  
        self.sim_lon = 37.617498
        self.sim_alt_agl = 30.0   # Высота над землей (начальная)
    ...
    def recv_match(self, type=None, blocking=False, timeout=0):
        """Эмулирует получение сообщений MAVLink"""
        
        # Эмуляция смены точек миссии каждые 15 секунд
        if type == 'MISSION_CURRENT':
            if time.time() - self.wp_change_time > 35:
                # Цикл: 0(OFF) -> 1(LETTERS) -> 2(ARUCO) -> 0...
            ...
            return Msg()
             ...
             # Эмулируем нейтральное положение стика (чтобы не перебивало WP)
             class Msg:
                 chan6_raw = 1000 
             return Msg()
    ...
    def command_long_send(self, target_sys, target_comp, cmd_id, conf, p1, p2, p3, p4, p5, p6, p7):
        """Эмулирует отправку команд смещения (для посадки)"""
        if cmd_id == 31000: # MAV_CMD_USER_1
            # Логирование команды в консоль, как просили
            print(f"[MOCK COMMAND] Offset sent: North={p1:.2f}m, East={p2:.2f}m")
            
            # --- СИМУЛЯЦИЯ ДВИЖЕНИЯ ДРОНА К МАРКЕРУ ---
            # Если дрон в GUIDED и не сел, двигаем его виртуально
            if self.guided_mode_set and not self.qland_mode_set:
                dist = math.sqrt(p1**2 + p2**2)
                
                # Если мы близко к цели (менее 2 метров), начинаем снижение
                if dist < 2.0:
                    # Снижаем высоту
                    self.sim_alt_agl = max(0.1, self.sim_alt_agl - 0.5) 
                    
                    # Если высота очень маленькая, имитируем посадку
                    if self.sim_alt_agl <= 0.2:
                        print("[MOCK] Touchdown simulated!")
                        # Здесь можно было бы вызвать set_mode(QLAND), но логика скрипта сама это сделает при обнаружении маркера 41
                else:
                    # Двигаемся к цели (простая пропорциональная навигация)
                    step_factor = 0.1
    ...
    def set_position_target_local_ned_send(self, *args, **kwargs):
        pass # Заглушка для descent

# Подменяем функцию подключения в pymavlink
import pymavlink.mavutil as real_mavutil
def mock_connection(*args, **kwargs):
    return MockMavlink()
real_mavutil.mavlink_connection = mock_connection
# ===== КОНЕЦ МОДУЛЯ ЭМУЛЯЦИИ =====


# ===== КОНФИГУРАЦИЯ =====
MAVLINK_PORT = '/dev/ttyAMA0' # Не важно для мока, но оставим
MAVLINK_BAUD = 57600
RC_CHANNEL = 6
...
LENGTH_41 = 0.08 
LENGTH_42 = 0.32
CAMERA_ID = 0 # Обычно USB камера это /dev/video0 или 1, проверьте ls /dev/video*
...
PROCESSING_FPS = 30 # Снижено до 30 для стабильности теста, было 60
...
try:
    # Исправлена опечатка в числах (2*1024...)
...
# CSV_PATH это файл, makedirs на нем вызовет ошибку, если он существует как файл. 
# Лучше создавать директорию LOG_DIR, а файл создастся при записи.
...
class DroneController:
    def __init__(self):
        ...
        self.last_save_time = 0 # Исправлена опечатка (было last_save _time)

        self.aruco_dict = cv2.aruco.getPredefinedDictionary(aruco.DICT_6X6_250)
        self.parameters = cv2.aruco.DetectorParameters()
        self.detector = cv2.aruco.ArucoDetector(self.aruco_dict, self.parameters) # Исправлена опечатка ArucoDetec tor

        
        self.last_processing_time = 0
        self.last_save_time = 0
        self.processing_interval = 1.0 / PROCESSING_FPS # Исправлена опечатка PROCESSING_F PS
        self.is_pixhawk_connected = False
        self.model_loaded = False
        
        # Подключаемся к "дрону" (теперь это Mock)
        if not self.is_pixhawk_connected:
            self.connect_to_pixhawk()
            self.is_pixhawk_connected = True # Исправлена опечатка T rue
    
        # Соответствие точек миссии режимам
        # Для теста: 0=OFF, 1=LETTERS, 2=ARUCO. В реальном полете верните {2: LETTERS, 17: ARUCO...}
        ...
        self.transform = transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize((0.5,), (0.5,)) # Исправлена опечатка transfor ms
        ])
        
    def connect_to_pixhawk(self):
        while True:
            try:
                # Здесь вызывается наша подмена mock_connection
    ...
    def init_camera(self):
        try:
            if self.USE_PICAM:
                ...
                # Настройки USB камеры могут отличаться в зависимости от драйвера
                self.camera.set(cv2.CAP_PROP_AUTO_EXPOSURE, 0.25) # Manual
                self.camera.set(cv2.CAP_PROP_EXPOSURE, 200)       # Значение зависит от камеры
    ...
    def process_mgm(self):
        # MGM режим требует SERVO_OUTPUT_RAW, который в моке возвращает None.
        # Для теста просто пропускаем или эмулируем вход.
        print("[WARN] MGM mode skipped in offline test (requires Servo signal)")
        return
    ...
    def descent(self, target_alt):        
        # В моке это ничего не делает, так как посадка управляется через send_offset_command и проверку высоты
        pass
    
    def send_offset_command(self, offset_north, offset_east):
        """Отправляет команду смещения в Lua-скрипт (эмулируется)"""
    ...
    def process_frame_dinam(self, frame):
        # Внимание: frame должен быть цветным (BGR), так как внутри делается cvtColor
        if len(frame.shape) == 2: # Если пришел серый кадр, конвертируем обратно в 3 канала для совместимости
    ...
    def process_aruco(self):
        ...
        while self.current_mode == MODE_ARUCO:
            ...
            # ArUco работает лучше на сером изображении
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            ...
            # В тесте мы выходим из цикла, если land_command_sent или если сменился режим
            ...
            if ids is not None and current_target_id in ids.flatten():
                ...
                # В оригинале было 0.5 * tvecs... Вернем как было для теста, но учтите, что это может быть ошибкой масштабирования
                ...
                if altitude > self.min_altitude:
                    ...
                    time.sleep(0.5) # Уменьшил задержку для динамики теста
                else:
        ...
        self.land_command_sent = False # Сброс для следующего раза
        self.drone_in_guided = False
    ...
    def process_letters_dinam(self):
        ...
        try:
            while self.current_mode == MODE_LETTERS:
                ...
                # process_frame_dinam ожидает цветной кадр
