...
CAMERA_RESOLUTION_PICAM = (1920, 1080)
CAMERA_RESOLUTION_USBCAM = (720, 480)
...
DROP_SERVO_ID = 8
DROP_SERVO_PWM_RELEASE = 2000  # Значение PWM для сброса груза
...
class DroneController:
    ...
    def connect_to_pixhawk(self):
        while True:
            try:
                ...
                logging.info(f"Connection: {self.master.target_system}")
                self.send_ssh_message(f"Connection:  {self.master.target_system}")

                break
            except Exception as e:
                logging.error(f"Failed to connect  Pixhawk: {e}. Repeat 5 s...")
                self.send_ssh_message(f"Failed to connect  Pixhawk {e}. Repeat 5 s...") 
        ...
        try:
            if self.USE_PICAM:
                ...
                try:
                    ...
                    logging.info("PiCamera is launched with sharpening")
                    self.send_ssh_message("PiCamera is launched with sharpening") 
                except:
                    logging.info("PiCamera is launched with out sharpening")
                    self.send_ssh_message("PiCamera is launched with out sharpening")
                    
            elif self.USE_USB_CAM:
                self.camera = cv2.VideoCapture(CAMERA_ID)
                if not self.camera.isOpened():
                    logging.error("Unable to open USB camera")
                    self.send_ssh_message("Unable to open USB camera")
                    self.camera = None
                    return
                ...
                try:
                    ...
                    logging.info("USB camera is running with settings")
                except:
                    logging.info("USB camera is running with out settings")
    ...
    def process_mgm(self):
        while True:
            ...
            if servo_pwm > DROP_SERVO_PWM_THRESHOLD:
                ...
                """Режим детекции МГМ (магнитного грунтомера)"""
    ...
    def execute_landing(self, offset_north, offset_east, altitude): 
       #print("\n=== Начало процедуры посадки ===")
        ...
        start_time = time.time()
        last_command_time = 0
        command_interval = 0.5  # Интервал между командами (сек)
        ...
        try:
            try:
                ...
                logging.error(f"Ошибка установки режима GUIDED: {e}")
            ...
            while not self.land_command_sent:
                ...
                if current_time - start_time > 30: 
                    self.send_ssh_message("Таймаут посадки! Активация QLAND")
    ...
    def process_aruco(self):
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Разрешение камеры: {CAMERA_RESOLUTION_USBCAM}")
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Частота обработки: {PROCESSING_FPS} FPS")
        
        last_save_time = time.time()
        last_command_time = 0
        command_interval = 0.1  # Интервал между командами (сек)
        ...
        while self.current_mode == MODE_ARUCO:
            ...
            if not self.drone_in_vtol:
                ...
                print("Установлен режим QSTABILIZE")
            ...
            if ids is not None and current_target_id in ids.flatten():
                ...
                # Отладочная информация
                self.send_ssh_message(f"Маркер {current_target_id} | Высота: {altitude:.2f}m | N: {offset_north:.2f} | E: {offset_east:.2f}")
    ...
    def process_letters(self):
        ...
        try:
            while self.current_mode == MODE_LETTERS:
                ...
                if frame is None:
                    time.sleep(0.1)
                    self.send_ssh_message("Кадр не полуучен")
                    print("Кадр не полуучен") 
                    continue
