...
class DroneController:
    ...
    def execute_landing(self, offset_north, offset_east): 
        ...
        try:
             while not self.land_command_sent :
                ...
                n_offset = offset_north 
                e_offset = offset_east
    ...
    def switch_to_vtol_mode(self):
        """
        Переводит QuadPlane в режим VTOL (вертикальный полёт)
        Возвращает True при успешном переходе, False при ошибке
        """
        # Отправляем команду перехода в VTOL
        self.master.mav.command_long_send(
            self.master.target_system,
            self.master.target_component,
            mavutil.mavlink.MAV_CMD_DO_VTOL_TRANSITION,
            0,          # confirmation
            3,          # param1: 3 = VTOL mode 4 - Режим самолёта
            0, 0, 0, 0, 0, 0, 0  
        )
    ...
    def process_aruco(self):
        ...
        while self.current_mode == MODE_ARUCO:
            ...
            if new_mode != self.current_mode:
                self.send_ssh_message(" Выход из режима ARUCO (Произошло в функции ARUCO)")
                self.current_mode = new_mode
                return # Выход из функции и переход в run 
            ...
            if ids is not None and MARKER_ID in ids.flatten():
                ...
                offset_north, offset_east = DroneController.calculate_offset(rvecs[0], tvecs[0])
    ...
    def process_letters(self):
        ...
        print("=== Активирован режим распознавания букв ===")
        self.send_ssh_message(f"Используется порог уверенности: {CONFIDENCE_THRESHOLD}")
        print(f"Используется порог уверенности: {CONFIDENCE_THRESHOLD}")
        self.send_ssh_message(f"Частота обработки: {PROCESSING_FPS} FPS")
        print(f"Частота обработки: {PROCESSING_FPS} FPS")
        ...
        try:
            while self.current_mode == MODE_LETTERS:
                new_mode = self.check_mode()
                if new_mode != self.current_mode:
                    self.send_ssh_message(" Выход из режима LETTERS (Произошло в LETTERS)")
                    self.current_mode = new_mode
        ...
        try:
            while True:
                ...
                if self.current_mode == MODE_OFF:
                    ...
                    self.master.mav.command_long_send(self.master.target_system, self.master.target_component, mavutil.mavlink.MAV_CMD_DO_SET_MODE,
                                                      0, mavutil.mavlink.MAV_MODE_FLAG_CUSTOM_MODE_ENABLED, 15, 0, 0, 0, 0, 0)
