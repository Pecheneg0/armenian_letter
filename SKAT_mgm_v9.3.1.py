...
CSV_PATH = "/media/pi/Smartbuy/objects-coordinates.csv" 
...
class DroneController:
    ...
    def add_meters_to_coords(self, lat, lon, dx_m, dy_m, yaw_deg=0, alt_amsl=0):
        """
    ...
    def process_frame_dinam(self, frame):
        ...
        current_lat, current_lon, alt_agl, alt_amsl = self.get_coordinates() 
        yaw = self.get_yaw()
        ...
        alt_safe = max(alt_agl, 1.0) # Защита от деления на ноль
        ...
        for cnt in contours:
            ...
            dx_m = raw_dx_m
            dy_m = raw_dy_m + alt_agl  # Используем честную высоту над землёй для смещения
            
            # Прибавляем метры к координатам дрона (3D пересчёт по ГОСТу с учетом аномальной высоты)
            new_lat, new_lon = self.add_meters_to_coords(current_lat, current_lon, dx_m, dy_m, yaw_deg=yaw, alt_amsl=alt_amsl)
