        ...
        try:
            if self.USE_PICAM:
                ...
                try:
                    ...
                    self.send_ssh_message("PiCamera запущена с настройками резкости") 
                except:
                    
                    logging.info("PiCamera запущена без настройек резкости")
                ...
                try:
                    ...
                    self.camera.set(cv2.CAP_PROP_EXPOSURE, 100)     # Уменьшаем экспозицию (значение подбирается экспериментально)
                    self.camera.set(cv2.CAP_PROP_BRIGHTNESS, 30000)   # Средняя яркость
                    self.camera.set(cv2.CAP_PROP_CONTRAST, 10000)     # Увеличиваем контраст
                    self.camera.set(cv2.CAP_PROP_SATURATION, -1000)   # Уменьшаем насыщенность
                    self.camera.set(cv2.CAP_PROP_SHARPNESS, 1000)   # Максимальная резкость
                    self.camera.set(cv2.CAP_PROP_GAIN, 1000)          # Минимальное усиление
                    logging.info("USB камера запущена с дополнительными настройками")
                except:
