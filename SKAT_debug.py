...
class DroneController:
    ...
    def process_aruco(self):
        ...
        print("=== Начало поиска ArUco маркеров ===")
        self.send_ssh_message("Старт поиска ArUco маркеров")
        
        # Явно откройте камеру
        cap = cv2.VideoCapture("/dev/video0")
        if not cap.isOpened():
            self.send_ssh_message("ОШИБКА: Не удалось открыть /dev/video1")
            return
        ...
        while self.current_mode == MODE_ARUCO:
            try:
                ...
                ret, frame = cap.read()
                if not ret:
                    self.send_ssh_message("Ошибка чтения кадра")
                    time.sleep(0.1)
                    continue
                ...
                # Отображение всех обнаруженных маркеров
                if ids is not None:
                    # Вывод в консоль не чаще 1 раза в секунду
                    if time.time() - last_print_time > 1.0:
                        self.send_ssh_message(f"Найдены маркеры: {ids.flatten()}")
                        last_print_time = time.time()
                    ...
                    for i, marker_id in enumerate(ids.flatten()):
                        ...
                        rvec, tvec, _ = cv2.aruco.estimatePoseSingleMarkers(
                            corners[i], 0.1, camera_matrix, dist_coeffs)
                ...
                error_msg = f"Ошибка в обработке: {str(e)}"
                self.send_ssh_message(error_msg)
                print(error_msg)
                time.sleep(1)
        ...
        print("=== Завершение работы ArUco ===")
        self.send_ssh_message("Завершение работы ArUco")
