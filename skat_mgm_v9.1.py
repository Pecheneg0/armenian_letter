...
class DroneController:
    ...
    def handle_mode_switch(self, new_mode):
        ...
        if new_mode_str != self.current_mode_str:
            logging.info(f" Переключение режима: {self.current_mode_str} -> {new_mode_str}")
    ...
    def execute_landing(self, offset_north, offset_east, altitude): 
        ...
        try:
            ...
            while not self.land_command_sent:
                ...
                #(f"Текущая высота: {altitude:.2f} m")
                ...
                if altitude > self.min_altitude:
                    if abs(offset_north) < self.pos_tolerance and abs(offset_east) < self.pos_tolerance:
                        logging.info("Малые смещения -> Снижение")
                        #self.send_ssh_message("Малые смещения -> Снижение") 
                        self.move_to_offset(0, 0, 0.6)

                    else:
                        logging.info(f"Коррекция: Север={offset_north:.2f}m | Восток={offset_east:.2f}m")
                    ...
                    if abs(offset_north) < self.pos_tolerance and abs(offset_east) < self.pos_tolerance:
                        ...
                        logging.info("Команда посадки отправлена, режим QLAND")
